...
from wabApp.models import AdminModel
from sqlalchemy.sql import exists
from wabApp.forms import AdminLoginForm, AdminProfileForm, AdminAddForm, Rules
...
file_handler = logging.FileHandler('wabApp/logs/admin.log')
    ...
    if request.method == 'GET':
        if 'admin_id' in session:
            if session.get('permission') == 1:
                ...
                flash('用户权限不足！')
                logger.warning(f"Admin ID {session.get('admin_id')} attempted to add an admin without sufficient "
                               f"permissions")
        ...
        if form.validate():
            ...
            if scalar.scalar():
                ...
                return render_template('pages_admin/lyear_pages_add_admin.html', redirect_url=url_for('admin'
                                                                                                      '.manage_admin'))
        else:
            flash('用户名或密码格式错误！', 'danger')
            logger.warning(f"Admin ID {session.get('admin_id')} provided invalid username or password format for new "
                           f"admin")
            return render_template('pages_admin/lyear_pages_add_admin.html')
...
def change_avatar():
    if 'admin_id' in session:
        ...
        if admin:
            ...
            if file and Rules.avater_allowed_file(file.filename):
